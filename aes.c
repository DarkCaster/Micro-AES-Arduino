...
#define SUB4(x)  (((uint32_t)(READ_FLASH_MEMORY(s_box+(((x) & 0xFF000000) >> 24)))<<24) | ((uint32_t)(READ_FLASH_MEMORY(s_box+(((x) & 0xFF0000) >> 16))) << 16) | ((uint32_t)(READ_FLASH_MEMORY(s_box+(((x) & 0xFF00) >> 8))) << 8) | (uint32_t)(READ_FLASH_MEMORY(s_box+((x) & 0xFF))))

// AES Tables

static const uint8_t s_box[256] TARGET_FLASH_MEMORY = {
...
static const uint8_t s_box_inverse[256] TARGET_FLASH_MEMORY = {
...
static const uint8_t rcon[10] TARGET_FLASH_MEMORY = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };
...
void aes_init(uint32_t * const round_key, counter b, const uint32_t * const key, counter n)
{
  ...
  for (position = n, cycle = n, t = round_key[n - 1], i = 0; position < b; position++, cycle++) {
    if (cycle == n) {
      ...
      t = SUB4(t);
      ((uint8_t *) &t)[0] ^= READ_FLASH_MEMORY(rcon+(i++));
    } else if (cycle == 4 && n == 8) {
      t = SUB4(t);
    }
...
void aes_SubBytes(uint8_t * const state)
{
  ...
  while (i--) {
    state[i] = READ_FLASH_MEMORY(s_box+(*(state+i)));
  }
}
...
void aes_InvSubBytes(uint8_t * const state)
{
  ...
  while (i--) {
    state[i] = READ_FLASH_MEMORY(s_box_inverse+(*(state+i)));
  }
}
